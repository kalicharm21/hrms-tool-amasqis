...
export const updatePipeline = async (companyId, pipelineId, updateData) => {
  try {
    ...
    if (!ObjectId.isValid(pipelineId)) {
      console.error("[PipelineService] Invalid ObjectId format", { pipelineId });
      return { done: false, error: "Invalid pipeline ID format" };
    }
    
    // First check if pipeline exists and log its details
    const existingPipeline = await collections.pipelines.findOne({ _id: new ObjectId(pipelineId) });
    console.log("[PipelineService] existing pipeline found", { existingPipeline });
    ...
    const updatedPipeline = await collections.pipelines.findOne({ _id: new ObjectId(pipelineId) });
...
export const deletePipeline = async (companyId, pipelineId) => {
  try {
    ...
    // Delete the pipeline
    const result = await collections.pipelines.deleteOne({ _id: new ObjectId(pipelineId) });
...
export const exportPipelinesPDF = async (companyId) => {
  try {
    ...
    doc.fontSize(10)
       ...
       .text('Total Deal Value', 50 + columnWidth, tableTop, { width: columnWidth })
       .text('No. of Deals', 50 + columnWidth * 2, tableTop, { width: columnWidth })
    ...
    pipelines.forEach((pipeline, index) => {
      ...
      doc.fontSize(9)
         .font('Helvetica')
         .fillColor(secondaryColor)
         .text(pipeline.pipelineName || 'N/A', 50, currentY, { width: columnWidth })
         .text(pipeline.totalDealValue != null ? `$${pipeline.totalDealValue}` : 'N/A', 50 + columnWidth, currentY, { width: columnWidth })
         .text(pipeline.noOfDeals != null ? pipeline.noOfDeals.toString() : 'N/A', 50 + columnWidth * 2, currentY, { width: columnWidth })
         .text(pipeline.stage || 'N/A', 50 + columnWidth * 3, currentY, { width: columnWidth })
         .text(pipeline.status || 'N/A', 50 + columnWidth * 4, currentY, { width: columnWidth })
         .text(formatDate(pipeline.createdDate), 50 + columnWidth * 5, currentY, { width: columnWidth });
  ...
  try {
    ...
    pipelines.forEach(pipeline => {
      worksheet.addRow({
        ...
        totalDealValue: pipeline.totalDealValue != null ? pipeline.totalDealValue : 'N/A',
    ...
    const totalRow = worksheet.addRow({
      pipelineName: 'TOTAL',
      totalDealValue: pipelines.reduce((sum, p) => sum + (p.totalDealValue || 0), 0),
